...
app
 ...
 .then(() => {
  ...
  .on('error',function(){ console.log("Error event")});
  ...
  function docVer(document1){
    ...
    DocModel.findOne( { 'id': document1.id },function (err, dbDocument) {
     ...
     dbDocument.docstatus = document1.docstatus;
     dbDocument.insstatus = document1.insstatus;
...
function saveDoc(document) {
 DocModel.findOne( {'owner': document.owner, 'id':document.id },function (err, dbDocument) {
   ...
   var p = new DocModel({owner: document.owner, id: document.id,
    imageLink: document.imageLink, docstatus: document.docstatus,
   uploadTime:document.uploadTime, insstatus:document.insstatus});
...
var file = 'C:\\Users\\ksred\\Downloads\\ZzZ\\Doc-Keeper-master\\filezz\\'+randd+'.pdf';
 ...
 server.get('/verifydocs', function(req, res){
  DocModel.find({ 'docstatus': 'unverified'}, function (err, docs) {
    var dox = [];
    var dox1 =[];
    var i;
   console.log("Number of Unverified Docs :",docs.length);
   for(i=0;i<docs.length;i++){
     dox.push(docs[i].imageLink)
     dox1.push(docs[i].id)
   }
   console.log("Dox :", dox);
 ...
 server.get('/verifydocs1', function(req, res){
  DocModel.find({ 'insstatus': 'unverified'}, function (err, docs) {
    var dox = [];
    var dox1 =[];
    var i;
   console.log("Number of Unverified Docs :",docs.length);
   ...
   console.log("Dox :", dox);
   return app.render(req, res, '/verify1', { ipfs:dox ,ids:dox1 } )
  })
 })
 ...
 return app.render(req, res, '/verify', { ipfs:dox ,ids:dox1 } )
  ...
  server.post('/mydocs', function(req, res){
     ...
     DocModel.find({ 'owner': account}, function (err, docs) {
      ...
      for(i=0;i<docs.length;i++){
        ipfs.push(docs[i].imageLink)
        docstatus.push(docs[i].docstatus)
        insstatus.push(docs[i].insstatus);
      }

      return app.render(req, res, '/mydocs', { ipfs:ipfs,docstatus:docstatus,insstatus:insstatus } );
     })
  })
